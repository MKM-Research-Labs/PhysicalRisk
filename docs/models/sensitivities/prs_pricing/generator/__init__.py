...
def generate():
    ...
    terrain_zones = [
        ...
        ('Zone 3a', 0.020, r'High ($>$1 in 100)'),
        ('Zone 3',  0.020, r'High (generic)'),
        ('Zone 2',  0.005, '1 in 100 to 1 in 1{,}000'),
        ('Zone 1',  0.001, r'$<$1 in 1{,}000'),
    ]
    # Representative property: 500 m from gauge, 1 m above gauge
    rep_dist = 500
    rep_elev = 1.0
    rep_ret = compute_retention(rep_dist)
    rep_elev_factor = math.exp(-rep_elev / ELEV_SCALE_M)

    rows = []
    for zone_name, zone_rate, _desc in terrain_zones:
        g_spread = compute_prs_spread(zone_rate, tenor=5, recovery=0.0,
                                       risk_free_rate=0.03)
        prop_rate = zone_rate * rep_ret * rep_elev_factor
    ...
    t7 = latex_table(
        ...
        r'($d=500$\,m, $\Delta h=1$\,m, $T=5$yr, $R=0$, $r=0.03$).',
        'sens_terrain',
