...
- **construction** (structural-frame combustibility — the building-material conflagration leg): `combustibility_by_type` maps `Construction.ConstructionType` to a fraction `c` in [0,1] (Reinforced concrete 0.00, Concrete frame 0.05, Steel frame 0.10, Brick and block 0.10, Stone/Masonry 0.12, Modern methods 0.25, Mixed construction 0.50, Timber frame 1.00); `default_combustibility` 0.50 for unknown; `combustible_threshold` 0.30; `intensity_ceiling` [5.0,120.0] (threshold-anchored — every non-combustible frame shares the 5.0 plateau, which sits below the worst i_crit, while combustible frames grade up); `structural_containment_weight` 1.0 (a non-combustible frame floors the containment-matrix blend weight at `(1-c)`, so it is contained passively even when suppression cannot reach it); `growth_combustibility_scale` [0.5,1.5] (combustibility scales the per-step growth — the timing leg, neutral at the midpoint). `race_jitter.ceiling_sigma` 0.10 jitters the fuel cap per fire so a non-combustible frame with no suppression keeps a small residual conflagration tail ("eventually it catches") rather than being immortal. Net: building **material** — not height alone — governs the high-rise conflagration leg, via three legs (growth timing, fuel cap + residual, passive containment).
