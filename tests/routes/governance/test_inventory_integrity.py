...
"""
Structural integrity tests for the real data/model_inventory.json.

These tests read the ACTUAL inventory file (not a test fixture) and assert that
every model entry has the correct field types required by the governance API.
A model added with malformed fields (e.g. test_coverage as a string instead of
a dict) will cause GET /api/v1/governance/models to return a 500 error.
"""
...
class TestAPIEndpointAvailability:
    ...
    def test_real_inventory_endpoint_succeeds(self, monkeypatch):
        ...
        if any model has test_coverage or overall_risk_rating as a string,
        models.py line ~101 calls .get() on a string → AttributeError → 500.
        """
        import copy
        import pathlib
