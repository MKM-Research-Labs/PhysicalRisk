...
class TestGaugetsRandom:
    def test_calculate_water_level_returns_float(self):
        ...
        gauge = {"FloodAlert": 3.0, "FloodWarning": 4.0, "SevereFloodWarning": 5.0}
        v = calculate_water_level(gauge, hour=12, storm_active=False)
    ...
    def test_determine_alert_status_thresholds(self):
        ...
        gauge = {"FloodAlert": 3.0, "FloodWarning": 4.0, "SevereFloodWarning": 5.0}
        assert determine_alert_status(2.0, gauge) == "Normal"
        assert determine_alert_status(3.5, gauge) == "Alert"
        assert determine_alert_status(4.5, gauge) == "Warning"
        assert determine_alert_status(5.5, gauge) == "Severe"
    ...
    def test_generate_gauge_reading_shape(self):
        ...
        gauge = {
            "GaugeID": "GAUGE-h001",
            "FloodAlert": 3.0, "FloodWarning": 4.0, "SevereFloodWarning": 5.0,
        }
        reading = generate_gauge_reading(gauge, datetime(2026, 1, 1, 12, 0))
        assert "GaugeID" in reading or "gauge_id" in reading

    def test_generate_time_series_returns_list(self, gauges_for_timeseries):
        from port.rand.halong.gauge.gaugets_random import generate_time_series
        result = generate_time_series(
            gauges_for_timeseries, num_hours=4, storm_active=False
        )
