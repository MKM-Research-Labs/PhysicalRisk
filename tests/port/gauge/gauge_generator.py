...
def _iso_catchment(tmp_path):
    """Route the migrated gauge writer's reads/writes to a scratch dir.

    The WP2.4 gauge generator no longer takes ``output_dir``; it persists through
    ``database.save_gauges(active_catchment(), …)``. Binding a tmp-rooted backend
    isolates every test and lets file assertions read back via ``database`` (or by
    inspecting ``tmp_path / 'gauge.json'``, since the backend roots there).
    """
...
class TestGaugeGeneratorFile:
    ...
    def test_output_persisted(self, tmp_path):
        ...
        assert database.get_gauge_portfolio(gen.catchment) is not None

    def test_output_is_valid_portfolio(self, tmp_path):
        gen = GaugePortfolioGenerator(verbose=False)
        gen.generate(count=5)
        data = database.get_gauge_portfolio(gen.catchment)
        assert "flood_gauges" in data

    def test_output_has_correct_count(self, tmp_path):
        gen = GaugePortfolioGenerator(verbose=False)
        expected = min(5, len(gen.params.GAUGE_POINTS))
        gen.generate(count=5)
        data = database.get_gauge_portfolio(gen.catchment)
...
class TestGaugeGeneratorEdgeCases:
    ...
    def test_generate_gauges_convenience_function(self, tmp_path):
        ...
        result = generate_gauges(count=3)
        assert "data" in result
...
class TestGaugeGeneratorDataQuality:
    ...
    def test_save_exception_propagates(self, tmp_path):
        """A failure in the database save is logged and re-raised."""
        from unittest.mock import patch
        gen = GaugePortfolioGenerator(verbose=False)
        with patch('database.save_gauges', side_effect=IOError('disk full')):
...
class TestGaugeGeneratorMain:
    ...
    def test_main_block(self, tmp_path):
        """The __main__ block calls generate_gauges and logs results. The autouse
        tmp_catchment fixture roots the backend at tmp_path, so the write lands there."""
        import runpy
