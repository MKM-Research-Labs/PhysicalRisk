...
class TestCreateCompletePopupContent:
    ...
    def _build(self, builder, prop, address, flood_info=None,
               has_mortgage=False, mortgage_info=None, mortgage_risk_info=None):
    ...
    def test_no_mortgage_risk_section_when_none(self, builder, prop, address):
        html = self._build(builder, prop, address, mortgage_risk_info=None)
        assert 'MORTGAGE RISK ANALYSIS' not in html

    def test_mortgage_risk_section_present_when_supplied(self, builder, prop, address,
                                                          mortgage_risk_info):
        html = self._build(builder, prop, address, mortgage_risk_info=mortgage_risk_info)
...
class TestBuildPropertyPopup:
    ...
    def test_popup_with_all_optional_params(self, builder, prop, address,
                                            flood_info, mortgage_info, mortgage_risk_info):
        popup = self._popup(
    ...
    def test_popup_html_with_high_risk_contains_mortgage_risk(
            self, builder, prop, address, flood_info, mortgage_info, mortgage_risk_info):
        popup = self._popup(
            builder, prop, address,
            flood_risk='High',
            has_mortgage=True,
        ...
        assert 'MORTGAGE RISK ANALYSIS' in popup.html.render()
