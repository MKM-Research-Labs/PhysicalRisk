...
"""Tests for reports.property.property_page_13_risk_analysis — RiskAnalysisPage."""

from reportlab.platypus import Paragraph, Table


def _make_property(flood_risk="Medium"):
    return {
        "PropertyHeader": {
...
def _make_mortgage(ltv=0.65, in_arrears=False, missed_payments=0,
...
class TestRiskAnalysisPageBasics:
    ...
    def test_property_only_mode(self):
        result = self._page().generate_elements(_make_property(), mortgage_data=None)
...
class TestLTVRiskBranches:
    ...
    def test_very_high_ltv_over_95(self):
        result = self._page().generate_elements(_make_property(), _make_mortgage(ltv=0.97))
        assert isinstance(result, list)

    def test_high_ltv_90_to_95(self):
        result = self._page().generate_elements(_make_property(), _make_mortgage(ltv=0.92))
        assert isinstance(result, list)

    def test_medium_ltv_80_to_90(self):
        result = self._page().generate_elements(_make_property(), _make_mortgage(ltv=0.85))
        assert isinstance(result, list)

    def test_low_medium_ltv_70_to_80(self):
        result = self._page().generate_elements(_make_property(), _make_mortgage(ltv=0.75))
        assert isinstance(result, list)

    def test_low_ltv_under_70(self):
        result = self._page().generate_elements(_make_property(), _make_mortgage(ltv=0.60))
        assert isinstance(result, list)
...
class TestPaymentPerformanceBranches:
    ...
    def test_in_arrears_score_5(self):
        result = self._page().generate_elements(_make_property(), _make_mortgage(in_arrears=True))
        assert isinstance(result, list)

    def test_many_missed_payments_over_3(self):
        result = self._page().generate_elements(_make_property(), _make_mortgage(missed_payments=4))
        assert isinstance(result, list)

    def test_some_missed_payments_1_to_3(self):
        result = self._page().generate_elements(_make_property(), _make_mortgage(missed_payments=2))
        assert isinstance(result, list)

    def test_no_missed_payments_good(self):
        result = self._page().generate_elements(_make_property(), _make_mortgage(missed_payments=0))
        assert isinstance(result, list)
...
class TestCreditRiskBranches:
    ...
    def test_excellent_credit_800_plus(self):
        result = self._page().generate_elements(_make_property(), _make_mortgage(credit_score=820))
        assert isinstance(result, list)

    def test_very_good_credit_740_to_799(self):
        result = self._page().generate_elements(_make_property(), _make_mortgage(credit_score=760))
        assert isinstance(result, list)

    def test_good_credit_670_to_739(self):
        result = self._page().generate_elements(_make_property(), _make_mortgage(credit_score=700))
        assert isinstance(result, list)

    def test_fair_credit_580_to_669(self):
        result = self._page().generate_elements(_make_property(), _make_mortgage(credit_score=620))
        assert isinstance(result, list)

    def test_poor_credit_under_580(self):
        result = self._page().generate_elements(_make_property(), _make_mortgage(credit_score=500))
        assert isinstance(result, list)
...
class TestInterestRateBranches:
    ...
    def test_high_rate_over_7(self):
        result = self._page().generate_elements(_make_property(), _make_mortgage(lending_rate=8.0))
        assert isinstance(result, list)

    def test_elevated_rate_5_to_7(self):
        result = self._page().generate_elements(_make_property(), _make_mortgage(lending_rate=6.0))
        assert isinstance(result, list)

    def test_moderate_rate_3_to_5(self):
        result = self._page().generate_elements(_make_property(), _make_mortgage(lending_rate=4.0))
        assert isinstance(result, list)

    def test_low_rate_under_3(self):
        result = self._page().generate_elements(_make_property(), _make_mortgage(lending_rate=2.5))
        assert isinstance(result, list)

    def test_no_rate_unknown(self):
        result = self._page().generate_elements(_make_property(), _make_mortgage(lending_rate=None))
        assert isinstance(result, list)
...
class TestDTIBranches:
    ...
    def test_very_high_dti_over_43(self):
        ...
        result = self._page().generate_elements(
            _make_property(), _make_mortgage(income=60_000, current_payment=5000)
        )
        assert isinstance(result, list)
    ...
    def test_high_dti_36_to_43(self):
        ...
        result = self._page().generate_elements(
            _make_property(), _make_mortgage(income=72_000, current_payment=2500)
        )
        assert isinstance(result, list)
    ...
    def test_moderate_dti_28_to_36(self):
        ...
        result = self._page().generate_elements(
            _make_property(), _make_mortgage(income=72_000, current_payment=1800)
        )
        assert isinstance(result, list)
    ...
    def test_good_dti_under_28(self):
        ...
        result = self._page().generate_elements(
            _make_property(), _make_mortgage(income=72_000, current_payment=1000)
        )
        assert isinstance(result, list)

    def test_no_payment_dti_unknown(self):
        result = self._page().generate_elements(
            _make_property(), _make_mortgage(income=80_000, current_payment=None)
        )
        assert isinstance(result, list)
...
class TestOverallRiskLevels:
    ...
    def test_critical_risk_scenario(self):
        ...
        result = self._page().generate_elements(
            _make_property(flood_risk="Very High"),
            _make_mortgage(ltv=0.97, in_arrears=True, credit_score=500)
        )
        assert isinstance(result, list)
    ...
    def test_low_risk_scenario(self):
        ...
        result = self._page().generate_elements(
            _make_property(flood_risk="Very Low"),
            _make_mortgage(ltv=0.40, credit_score=820, missed_payments=0)
        )
        assert isinstance(result, list)

    def test_property_only_high_flood_risk(self):
        result = self._page().generate_elements(_make_property(flood_risk="High"), mortgage_data=None)
        assert isinstance(result, list)

    def test_property_only_very_low_flood_risk(self):
        result = self._page().generate_elements(_make_property(flood_risk="Very Low"), mortgage_data=None)
        assert isinstance(result, list)
