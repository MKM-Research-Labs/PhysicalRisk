...
"""
Unit tests for lineage prerequisite resolution — resolve_prerequisites()
and _outputs_exist().
"""
...
class TestResolvePrerequisites:
    ...
    def test_transitive_chain(self, tmp_path):
        ...
        # propertyts depends on properties + stressm
        # properties depends on gauges
        # stressm depends on gauges + gaugehd
        # gaugehd depends on gauges
        assert "gauges" in result
        assert "properties" in result
...
class TestPortPrereqIntegration:
    ...
    def test_single_step_sets_prereq_flags(self):
        ...
        # hazard should NOT be enabled (not a prereq of propertyts)
        assert args.hazard is False
