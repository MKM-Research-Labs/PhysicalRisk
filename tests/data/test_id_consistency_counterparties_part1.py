...
class TestCounterpartyTradeLinkage:
    ...
    @pytest.mark.xfail(
        reason="Counterparty data is being migrated to the database (governance→DB "
               "project); CTPY-* references are no longer guaranteed to resolve in "
               "the file-based counterparty.json. Deferred to that migration, not a "
               "test fix. Remove this marker once counterparties are seam-served.",
        strict=False,
    )
    ...
    @pytest.mark.xfail(
        reason="trade_marks.json is regenerated by `app.py port --blotter`; the "
               "current thames port has PRS trades not yet marked (>15% tolerance). "
               "Resolving needs a port regeneration in the main checkout (gated, and "
               "not run from a worktree on the shared SSD) — will XPASS once regenerated.",
        strict=False,
    )
