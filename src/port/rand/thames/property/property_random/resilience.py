...
sets the two Thames-specific toggles:
  ...
  - PUBLISH_BRI_LETTER_RATINGS = False  → Thames has no certified BRI letter
    regime, so generated properties report "NR". Read by the shared builder.
  - BRI_SCORES_ENABLED = False          → the five numeric BRIScore fields are
    stamped 0.0 (BRI is not a Thames concept).
...
# Thames-specific toggles (see module docstring).
