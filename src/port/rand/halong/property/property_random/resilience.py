...
"""
Thames property resilience — single source of truth.

This module is the consolidated home for everything resilience-related for the
Thames synthetic property pipeline. It contains, in order:
...
The Thames-specific calibration (weights, thresholds, priors) lives here
deliberately — when other catchments are added they will have their own copy
under ``src/port/rand/<catchment>/property/property_random/resilience.py``.
"""
...
def validate_weights() -> None:
    """Raise ``ValueError`` if ``SECTION_WEIGHTS`` does not sum to 1.0."""
