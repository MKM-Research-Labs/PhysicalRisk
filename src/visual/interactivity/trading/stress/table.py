...
"""
Stress Test — trade table sub-module.

Renders the trade table at peak P(flood) hour with stress P&L.
"""


def get_js() -> str:
    """Return JavaScript fragment for stress test trade table."""
    return """
            // ---- Trade table at peak hour ----
            function _tdRenderStressTable(data) {
                ...
                var stPnlColor = function(v) { return v >= 0 ? '#2e7d32' : '#c62828'; };
                ...
                trades.forEach(function(t) {
                    ...
                    html += '<tr style="border-bottom:1px solid #f0f0f0;' +
                        ...
                        '<td style="padding:3px 4px;text-align:center;color:' + dirColor + ';font-weight:600;">' +
                            dir + '</td>' +
                        '<td style="padding:3px 6px;text-align:right;">' + stFmtGBP(t.notional) + '</td>' +
                        '<td style="padding:3px 6px;text-align:right;color:' + stPnlColor(t.mtm) + ';">' +
                            stFmtGBP(t.mtm) + '</td>' +
                        '<td style="padding:3px 6px;text-align:right;color:' + stPnlColor(cashPrice) + ';">' +
                ...
                html += '<tr style="border-top:2px solid #333;background:#f8f9fa;font-weight:700;">' +
                    ...
                    '<td style="padding:4px 6px;text-align:right;">' + stFmtGBP(totalNotional) + '</td>' +
                    '<td style="padding:4px 6px;text-align:right;color:' + stPnlColor(totalMtm) + ';">' +
                        stFmtGBP(totalMtm) + '</td>' +
                    '<td style="padding:4px 6px;text-align:right;color:' + stPnlColor(totalCash) + ';">' +
                        stFmtGBP(totalCash) + '</td>' +
                    '<td style="padding:4px 6px;text-align:right;color:' + stPnlColor(totalStress) + ';">' +
