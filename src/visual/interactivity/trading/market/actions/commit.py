...
"""Market tab — sequential commit queue and chart cleanup."""


def get_js() -> str:
    """Return JS fragment for tdCommitMarket and tdCleanupMarketCharts."""
    return """
            window.tdCommitMarket = function() {
                ...
                function processNextCommit() {
                    if (commitIdx >= commitQueue.length) {
                        ...
                        if (btn) { btn.textContent = 'Commit'; btn.disabled = false; btn.style.background = '#1976d2'; }
