    ...
    def get_js(self) -> str:
        ...
        (function() {{
            ...
            function createPanel() {{
                ...
                pdfPanel.id = 'property-pdf-panel';
                ...
                title.id = 'property-pdf-title';
                title.style.cssText = 'font-weight:bold;font-size:14px;color:#333;';
                title.textContent = 'Property Report';

                var btnGroup = document.createElement('div');
                btnGroup.style.cssText = 'display:flex;gap:8px;';

                var downloadBtn = document.createElement('button');
                downloadBtn.id = 'property-pdf-download';
                downloadBtn.textContent = 'Download';
                downloadBtn.style.cssText =
                    'padding:4px 12px;border:1px solid #28a745;border-radius:4px;' +
                    'background:#28a745;color:white;cursor:pointer;font-size:12px;';
                downloadBtn.onclick = function() {{
                    var iframe = document.getElementById('property-pdf-iframe');
                    if (iframe && iframe.src) {{
                        var a = document.createElement('a');
                        a.href = iframe.src;
                        a.download = (title.textContent || 'property_report') + '.pdf';
                        a.click();
                    }}
                }};
            ...
            function showPanel(propertyId, pdfBase64) {{
                var panel = createPanel();
                var title = document.getElementById('property-pdf-title');
                title.textContent = 'Property Report: ' + propertyId;
            ...
            // Public API
            window.PropertyPDFPanel = {{
                show: showPanel,
                hide: hidePanel
            }};

            // Listen for property PDF results
            document.addEventListener('propertyPdfReady', function(e) {{
                if (e.detail && e.detail.propertyId && e.detail.pdfBase64) {{
                    showPanel(e.detail.propertyId, e.detail.pdfBase64);
                }}
            }});
        }})();
        </script>
