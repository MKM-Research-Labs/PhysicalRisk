...
function renderTestResultsTab(m) {
    var pdfUrl = '/api/v1/governance/models/' + m.model_id + '/test-results/pdf';
    ...
    html += '<div style="font-size:12px;color:#666;">Automated test results for model ' + m.model_id + '</div>';
    ...
    // Embedded PDF viewer
    html += '<div id="mg-test-pdf-container" style="flex:1;min-height:500px;border:1px solid #e0e0e0;border-radius:6px;overflow:hidden;">';
    ...
    html += '<div style="font-size:13px;font-weight:600;color:#333;margin-bottom:8px;">Test Results PDF</div>';
    ...
    // Check if PDF exists by attempting a HEAD request
    fetch(pdfUrl, {method: 'HEAD'}).then(function(resp) {
        if (!resp.ok) {
            var container = document.getElementById('mg-test-pdf-container');
            if (container) {
                container.innerHTML = '<div style="padding:40px;text-align:center;">' +
                    '<div style="font-size:32px;margin-bottom:12px;">&#x26A0;</div>' +
                    '<div style="font-size:13px;font-weight:600;color:#333;margin-bottom:8px;">No Test Results Available</div>' +
                    '<div style="font-size:11px;color:#888;margin-bottom:4px;">Test results PDF has not been generated yet.</div>' +
                    '<div style="font-size:11px;color:#888;">Run: <code style="background:#f5f5f5;padding:2px 6px;border-radius:3px;">python app.py check tests --pdf</code></div>' +
                    '</div>';
            }
        }
    });
...
function renderDocsTab(m) {
    var pdfUrl = '/api/v1/governance/models/' + m.model_id + '/documentation/pdf';
    var html = '<div style="display:flex;flex-direction:column;height:100%;">';

    // Header with download button
    html += '<div style="display:flex;align-items:center;justify-content:space-between;padding:8px 0;margin-bottom:8px;">';
    html += '<div style="font-size:12px;color:#666;">Model documentation for ' + m.model_id + ' &mdash; ' + m.name + '</div>';
    html += '<a href="' + pdfUrl + '" target="_blank" style="display:inline-flex;align-items:center;gap:6px;padding:6px 14px;background:#1976d2;color:white;border-radius:4px;text-decoration:none;font-size:11px;font-weight:500;">&#x2913; Download PDF</a>';
    html += '</div>';

    // Embedded PDF viewer
    html += '<div id="mg-doc-pdf-container" style="flex:1;min-height:500px;border:1px solid #e0e0e0;border-radius:6px;overflow:hidden;">';
    html += '<object data="' + pdfUrl + '" type="application/pdf" width="100%" height="500" style="border:none;">';
    html += '<div style="padding:40px;text-align:center;">';
    html += '<div style="font-size:32px;margin-bottom:12px;">&#x1F4C4;</div>';
    html += '<div style="font-size:13px;font-weight:600;color:#333;margin-bottom:8px;">Model Documentation PDF</div>';
    html += '<div style="font-size:11px;color:#888;margin-bottom:16px;">Your browser cannot display the PDF inline.</div>';
    html += '<a href="' + pdfUrl + '" target="_blank" style="display:inline-block;padding:8px 20px;background:#1976d2;color:white;border-radius:4px;text-decoration:none;font-size:12px;">Open PDF</a>';
    html += '</div>';
    html += '</object>';
    ...
    fetch(pdfUrl, {method: 'HEAD'}).then(function(resp) {
        if (!resp.ok) {
            var container = document.getElementById('mg-doc-pdf-container');
            if (container) {
                container.innerHTML = '<div style="padding:40px;text-align:center;">' +
                    '<div style="font-size:32px;margin-bottom:12px;">&#x26A0;</div>' +
                    '<div style="font-size:13px;font-weight:600;color:#333;margin-bottom:8px;">No Documentation Available</div>' +
                    '<div style="font-size:11px;color:#888;">Model documentation PDF has not been generated for ' + m.model_id + '.</div>' +
                    '</div>';
            }
        }
    });
