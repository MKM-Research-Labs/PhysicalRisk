...
class TCEventVisualization:
    ...
    def _add_flood_heatmap(self, base_map):
        ...
        gauge_storms = {}  # gauge_id -> {lat, lon, storm_peaks: {storm_id: peak}}
        ...
        pts_dir = self.input_dir / 'propertyts'
        prop_count = 0
        if pts_dir.exists():
        ...
        logger.info("Added worst-case flood heatmap (storm %s, %d gauges, %d properties)", worst_id, len(worst_points), prop_count)
